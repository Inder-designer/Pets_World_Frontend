...
// Make sure to type the mapping object correctly
const countryCodesToRegions: Record<Countries, Region[]> = {
    AF: [
        {
            "name": "Badakhshan",
            "shortCode": "BDS"
    ...
    SJ: [
        { name: "Svalbard", shortCode: "SV" },
        { name: "Jan Mayen", shortCode: "JM" },
    ]
}

export function getRegionsByCountryCode(code: Countries): Region[] {
    return countryCodesToRegions[code] || [];
}

// export function getRegionNamesByCountryCode(code: Countries): string[] {
//     return (countryCodesToRegions[code] || []).map(region => region.name);
// }

// export function getRegionShortCodesByCountryCode(code: Countries): string[] {
//     return (countryCodesToRegions[code] || [])
//         .map(region => region.shortCode)
